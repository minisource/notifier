...
func (s *TemplateService) UpdateTemplate(ctx context.Context, id uuid.UUID, updates *models.NotificationTemplate) (*models.NotificationTemplate, error) {
	...
	existing.ProviderTemplate = updates.ProviderTemplate
...
// GetTemplateByKey retrieves a template by its programmatic key
func (s *TemplateService) GetTemplateByKey(ctx context.Context, key string) (*models.NotificationTemplate, error) {
	template, err := s.templateRepo.GetByKey(ctx, key)
	if err != nil {
		s.logger.Error(logging.Postgres, logging.Select, "Failed to get template by key", map[logging.ExtraKey]interface{}{
			"error": err.Error(),
			"key":   key,
		})
		return nil, fmt.Errorf("failed to get template: %w", err)
...
// RenderPreview renders a template with the given variables for preview
func (s *TemplateService) RenderPreview(ctx context.Context, id uuid.UUID, variables map[string]string, locale string) (*RenderResult, error) {
	template, err := s.GetTemplate(ctx, id)
	if err != nil {
		return nil, err
	}

	// If no locale specified, use template's default
	if locale == "" {
		locale = template.Locale
	}

	result, err := RenderTemplate(&RenderRequest{
		Template:  template,
		Variables: variables,
		Locale:    locale,
	})
	if err != nil {
		s.logger.Error(logging.General, logging.Api, "Failed to render template preview", map[logging.ExtraKey]interface{}{
			"templateId": id,
			"error":      err.Error(),
		})
		return nil, err
	...
	s.logger.Debug(logging.General, logging.Api, "Template rendered for preview", map[logging.ExtraKey]interface{}{
		"templateId": id,
