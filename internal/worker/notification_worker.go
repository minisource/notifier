...
func (w *NotificationWorker) handleFailure(ctx context.Context, notification *models.Notification, job *NotificationJob, err error, processingTime int) {
	...
	if notification.RetryCount < notification.MaxRetries {
		...
		if err := w.notifRepo.UpdateStatus(ctx, notification.ID, models.NotificationStatusFailed); err != nil {
			w.logger.Error(logging.General, logging.Update, "Failed to mark notification as failed", map[logging.ExtraKey]interface{}{
				"notificationId": notification.ID,
				"error":          err.Error(),
			})
		}

		w.logger.Warn(logging.Internal, logging.Api, "Notification failed after max retries", map[logging.ExtraKey]interface{}{
			"notificationId": notification.ID,
			"retryCount":     notification.RetryCount,
		})

		// Create failure log
		w.createLog(ctx, notification.ID, "failed", models.NotificationStatusFailed, "Failed after max retries", err.Error())
	}
}
